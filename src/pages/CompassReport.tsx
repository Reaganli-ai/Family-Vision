...
function toViewData(cd: CompassDataSchema): CompassViewData {
  ...
  return {
    ...
    n_key_trend: (cd.N?.trends?.value as string[])?.join("、") || NOT_MENTIONED,
    n_core_competency: (cd.N?.coreAbility?.value as string[])?.join("、") || NOT_MENTIONED,
    w_story: (cd.W?.story?.value as string) || NOT_MENTIONED,
    w_core_spirit: cd.W?.spiritUpgrade?.value?.spirit || NOT_MENTIONED,
    w_spirit_from: cd.W?.spiritUpgrade?.value?.from || NOT_MENTIONED,
    w_spirit_to: cd.W?.spiritUpgrade?.value?.to || NOT_MENTIONED,
